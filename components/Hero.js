...
function Hero() {
    return (
        <section className="min-h-[70vh] flex flex-col-reverse md:flex-row items-center justify-center bg-white dark:bg-black transition-colors duration-500">
            {/* Left: Info */}
            <div className="flex flex-col justify-center items-center md:items-start px-4 sm:px-8 md:px-12 py-8 md:py-0">
            ...
            {/* Right: Photo */}
            <div className="flex items-center justify-center py-8 md:py-0 px-4">
                <div className="relative w-52 h-52 md:w-80 md:h-80 rounded-full bg-gradient-to-tr from-black via-gray-700 to-white p-1 dark:from-white dark:via-gray-300 dark:to-black shadow-2xl">
                    <Image src="/profile.jpg" alt="Profile" fill sizes="(max-width: 768px) 208px, 320px" className="rounded-full object-cover border-4 border-white dark:border-gray-400" />
                </div>
            </div>
        </section>
    );
